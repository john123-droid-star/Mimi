...
from token_refresher import refresh_all as _refresh_all, TOKEN_CACHE

app = Flask(__name__)

# ✅ Valid API keys
VALID_API_KEYS = {
    "Mr"  # matches like.py (base64 "TXI=")
}
...
# ─────────── Token loading (memory-first, file-fallback) ───────────
def load_tokens(region):
    # 1. Try in-memory cache first (populated by /refresh)
    if region == "IND":
        cache_key = "IND"
...
# 🆕 /refresh endpoint — regenerates JWT tokens
...
def manual_refresh():
    ...
    try:
        result = asyncio.run(_refresh_all())
